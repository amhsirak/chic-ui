...
var Select = function (_a, ref) {
    var className = _a.className, _b = _a.size, size = _b === void 0 ? 'default' : _b, disabled = _a.disabled, error = _a.error, multiSelect = _a.multiSelect, _c = _a.width, width = _c === void 0 ? '100%' : _c, data = _a.data, dataKey = _a.dataKey, dataName = _a.dataName, placeholder = _a.placeholder, style = _a.style, onChange = _a.onChange;
    ...
    var showSelectedOptionValue = function () {
        if (!multiSelect) {
            ...
            return ((selectedOption &&
                ...
                selectedOption.map(function (option) { return (_jsxs(MultiSelectOptionWrapper, { children: [dataKey && dataName ? option[dataName] : option, _jsx(StyledCross, { onClick: function (e) { return (e.stopPropagation, setMultiSelectOption(option)); } })] })); })) ||
                placeholder);
        }
    };
    return (_jsxs(SelectWrapper, __assign({ ref: ref, className: className, width: width, style: style }, { children: [_jsxs(StyledSelectDiv, __assign({ tabIndex: disabled ? undefined : 0, disabled: disabled || false, error: error || false, innerSize: size, onClick: function () { return !disabled && setShowOptions(!showOptions); } }, { children: [_jsx("div", { children: showSelectedOptionValue() }), _jsx(ArrowDown, {})] })), showOptions && (_jsx(SelectOptionWrapper, { children: data &&
                    data.map(function (option, index) { return (_jsx(SelectOption, __assign({ selected: multiSelect &&
                            ...
                            onOptionSelect(dataKey ? option[dataKey] : option)); } }, { children: dataKey && dataName ? option[dataName] : option }), "".concat(option, "_").concat(index))); }) }))] })));
};
