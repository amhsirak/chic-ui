...
> = (props, ref) => {
  ...
  const toggleHandler = (event: React.MouseEvent<HTMLSpanElement>) => {
    ...
    if (
      toggleRef?.current?.className.includes('open') &&
      descRef?.current?.className.includes('open')
    ) {
      toggleRef.current.className = 'accordion-toggle-button';
      descRef.current.className = descRef.current.className.replace(
        'open',
        'close'
      );
  ...
  return (
    <StyledAccordion ref={ref} className={className}>
      <StyledHeader className="accordion-header" onClick={toggleHandler}>
        <div ref={toggleRef} className="accordion-toggle-button">
          <span></span>
        </div>
        {header}
      </StyledHeader>
      <StyledDescriptionWrapper className="desc-wrapper">
        <StyledDescription
          className="accordion-description close"
          ref={descRef}
        >
          {description}
        </StyledDescription>
